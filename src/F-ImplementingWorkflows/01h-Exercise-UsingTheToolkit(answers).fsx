...
let createValidCustomer : CreateValidCustomer =
    fun dto ->
        ...
        let customerOrError =
            (Validation.map3 createCustomer) nameOrError emailOrError bdateOrError
...
let processCustomerDto (jsonOrError:Result<Json,WorkflowError>) :Result<Customer,WorkflowError> =
...
// Exercise: Eliminate the compiler errors
// by using Async.map and AsyncResult.bind
